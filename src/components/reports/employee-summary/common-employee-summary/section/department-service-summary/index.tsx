...
const DepartmentServiceSummary = (props: ITabEmployeeSummaryProps) => {
    const { tab, currentTab, dataTmaSummary = [] } = props;

    const moduleName = props.moduleName + 'DepartmentService';
    const moduleNameDetails = props.moduleName + 'DepartmentServiceDetails';
    ...
    const departmentSummaryColumns: ColumnType<IUnitSummary>[] = [
        {
            dataIndex: 'unitName',
            key: 'unitName',
    ...
    useEffect(() => {
        const fetchDataList = async (params: IParamsList) => {
            turnOnLoading();
            try {
                // Department Service Summary in DB is id 2
                const res = await employeeSummaryService.getUnitSummaryList({ ...params, customOrgChartReportStructureId: 2 }, moduleName);
                ...
                showNotification(false, 'Error fetching get department list');
            } finally {
                turnOffLoading();
            }
        };
    ...
    return (
        ...
        <div id={`${escapeSelector(dataTmaSummary[2]?.orgChartReportTypeName)}_${currentTab}`} className="title-header-table">
            <h3 className="title">Department Service Summary (Total Employee: {sumTotalEmployees(dataList)})</h3>
            <BaseTable
...
export default DepartmentServiceSummary;
