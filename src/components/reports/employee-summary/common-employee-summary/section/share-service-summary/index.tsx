...
interface IShareServiceSummaryProps extends ITabEmployeeSummaryProps {}

const ShareServiceSummary = (props: IShareServiceSummaryProps) => {
    const { tab, currentTab, dataTmaSummary = [] } = props;

    const moduleName = props.moduleName + 'SharedService';
    const moduleNameDetails = props.moduleName + 'SharedServiceDetails';
    ...
    const shareServiceSummaryColumns: ColumnType<IUnitSummary>[] = [
        {
            dataIndex: 'unitName',
            key: 'unitName',
    ...
    useEffect(() => {
        const fetchDataList = async (params: IParamsList) => {
            turnOnLoading();
            try {
                // Shared Service Summary in DB is id 3
                const res = await employeeSummaryService.getUnitSummaryList({ ...params, customOrgChartReportStructureId: 3 }, moduleName);
                ...
                showNotification(false, 'Error fetching get share service list');
            } finally {
                turnOffLoading();
            }
        };
    ...
    return (
        ...
        <div id={`${escapeSelector(dataTmaSummary[3]?.orgChartReportTypeName)}_${currentTab}`} className="title-header-table">
            <h3 className="title">Shared Service Summary (Total Employee: {sumTotalEmployees(dataList)})</h3>
            <BaseTable
                columns={shareServiceSummaryColumns}
...
export default ShareServiceSummary;
