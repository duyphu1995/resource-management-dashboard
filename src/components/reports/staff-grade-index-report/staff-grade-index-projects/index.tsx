...
import StaffGradeIndexReportProjectTable from './table';
interface StaffGradeIndexProjectsProps {
    filterParams: IStaffGradeFilterParams;
}

const StaffGradeIndexProjects = ({ filterParams }: StaffGradeIndexProjectsProps) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (filterParams.unitTypeLevel !== GroupLevel.Project.toString()) return;
            try {
                turnOnLoading();
                const { succeeded, data } = await staffGradeIndexReportService.getTableData(filterParams.year, filterParams.week, GroupLevel.Project);
                if (succeeded && data) {
    ...
    return (
        <div>
            ...
            <Title level={3} style={{ paddingTop: 24 }}>
                Staff Grade Index {ORG_UNITS.Project}
            </Title>
            <StaffGradeIndexReportProjectTable data={reportData} loading={isLoading} />
        </div>
    );
};

export default StaffGradeIndexProjects;
