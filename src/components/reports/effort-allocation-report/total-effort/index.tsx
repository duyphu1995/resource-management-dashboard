...
import { IEffortAllocationTable, IPropsSubTabs } from '@/types/reports/effort-allocation';
...
const TotalEffort: React.FC<IPropsSubTabs> = ({ setPayloadExport }) => {
    const { showNotification } = useNotify();

    const [totalEffortList, setTotalEffortList] = useState<IEffortAllocationTable[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const res = await effortAllocationReportServices.getEffortLocation({ filterBy: 'Effort', sortedBy: valueSortBy });
                const { data } = res;
                setTotalEffortList(data || []);
            } catch (error) {
        ...
        <DetailContent rootClassName="effort-tab">
            <div className="effort-tab__container">
                ...
                <div className="total-contractor">
                    ...
                    <h4>Contractors: {totalEffortList.filter(item => item.statusName === 'Contractor').length}</h4>
                </div>
            </div>
            <BaseTable columns={columns} dataSource={formatDataTable(totalEffortList)} loading={isLoading} rowClassName={rowClassName} bordered />
        </DetailContent>
    );
};

export default TotalEffort;
