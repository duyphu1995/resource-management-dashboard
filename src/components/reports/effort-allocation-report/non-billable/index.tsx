...
const NonBillable: React.FC<IPropsSubTabs> = ({ setPayloadExport }) => {
    const { showNotification } = useNotify();

    const [nonBillableList, setNonBillableList] = useState<IEffortAllocationTable[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const res = await effortAllocationReportServices.getEffortLocation({ filterBy: 'NonBillable', sortedBy: valueSortBy });
                const { data } = res;
                setNonBillableList(data || []);
            } catch (error) {
        ...
        <DetailContent rootClassName="effort-tab">
            <div className="effort-tab__container">
                ...
                <div className="total-contractor">
                    ...
                    <h4>Contractor: {nonBillableList.filter(item => item.statusName === 'Contractor').length}</h4>
                </div>
            </div>
            <BaseTable columns={columns} dataSource={formatDataTable(nonBillableList)} loading={isLoading} rowClassName={rowClassName} bordered />
        </DetailContent>
    );
};
