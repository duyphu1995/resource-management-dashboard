...
const TerminationReasonPage = () => {
    const { showNotification } = useNotify();
    const { havePermission } = usePermissions('AppendixEmployeeAppendixTerminationReasonList', 'TerminationReason');
    const { isLoading, turnOnLoading, turnOffLoading } = useLoading();
    const selectSearchParamsRedux = useAppSelector(selectSearchParams);
    const searchByKeywordFromRedux = selectSearchParamsRedux.adminTerminationReason.searchByKeyword;
    const paginationTableFromRedux = selectSearchParamsRedux.adminTerminationReason.paginationTable;

    const [keyword, setKeyword] = useState(searchByKeywordFromRedux || '');
    const [showDeleteDialog, setShowDeleteDialog] = useState(false);
    const [deletedData, setDeletedData] = useState<IAdminTerminationReason | null>(null);
    const [showDeleteWarningDialog, setShowDeleteWarningDialog] = useState(false);
    const [allData, setAllData] = useState<IAdminTerminationReason[]>([]);
    const [data, setData] = useState<IAdminTerminationReason[]>(allData);
    const [isReload, setIsReload] = useState<object>({});

    const deleteDialogTitle = 'Delete Termination Reason';
    const deleteWarningDialogTitle = 'Can’t Delete';

    const deleteWarningDialogContent = (
        <div style={{ width: 384 }}>
            Cannot delete Reason <strong>{deletedData?.reasonName}</strong>. Because this Reason has been using.
        </div>
    );
    const deleteDialogContent = (
        <div style={{ width: 384 }}>
            The Reason <strong>{deletedData?.reasonName}</strong> will be deleted. Are you sure you want to delete this Reason?
        </div>
    );

    const [columns] = useState<ColumnType<IAdminTerminationReason>[]>([
        {
            key: 'reasonName',
            title: 'Reason Name',
            width: 329,
            ...
            render: (record: IAdminTerminationReason) => (
                <Link
                    className="underline"
                    style={{ color: '#323232' }}
                    to={pathnames.admin.appendix.employeeAppendix.terminationReason.detail.path + '/' + record.reasonId}
                >
                    {renderWithFallback(record.reasonName)}
                </Link>
            )
        },
        {
            ...
            render: (record: IAdminTerminationReason) => renderWithFallback(formatTimeMonthDayYear(record.createdOn))
        },
        {
            key: 'lastModifiedOn',
            title: 'Edited Date',
            ...
            render: (record: IAdminTerminationReason) => renderWithFallback(formatTimeMonthDayYear(record.lastModifiedOn))
        },
        {
            key: 'action',
            title: 'Action',
    ...
    const onShowDeleteDialog = (deletedData: IAdminTerminationReason) => {
    ...
    const onDelete = async () => {
        ...
        try {
            const res = await terminationReasonService.delete(deletedData?.reasonId || 0);
    ...
    useEffect(() => {
        const fetchData = async () => {
            turnOnLoading();
            const res = await terminationReasonService.getAll();
            const { data = [] } = res;
            setAllData(data);
            turnOffLoading();
        };
    ...
    const { handlePageChange, setSearchByKeyword } = useDataTableControls('adminTerminationReason');
    ...
    return (
        <DetailContent>
            ...
            <DetailHeader pageTitle={pathnames.admin.appendix.employeeAppendix.terminationReason.main.name} buttons={[]} />

            <Flex justify="space-between" gap={'20px 22px'}>
                <SearchInput style={{ width: 357 }} placeholder="Search by Reasons" setKeyword={setKeyword} defaultValue={keyword} />
                {havePermission('Add') && (
                    <Link to={pathnames.admin.appendix.employeeAppendix.terminationReason.add.path}>
                        <Button type="primary">Add New Termination Reason</Button>
                    </Link>
                )}
            </Flex>
