...
const breadcrumbItems: BreadcrumbItemType[] = [
    ...
    { title: pathnames.admin.appendix.employeeAppendix.certification.detailCertificationName.name }
];

const CertificationNameDetailPage = () => {
    const { id = '' } = useParams();
    const navigation = useNavigate();
    const { havePermission } = usePermissions('DetailsCertificationNameAppendix', 'Certification');
    const { isLoading, turnOnLoading, turnOffLoading } = useLoading();

    const [data, setData] = useState<IAdminCertificationName | null>(null);

    const pageTitle = pathnames.admin.appendix.employeeAppendix.certification.detailCertificationName.name;

    const onGoBack = () => navigation(pathnames.admin.appendix.employeeAppendix.certification.main.path);
    const navigateToEditPage = () => navigation(pathnames.admin.appendix.employeeAppendix.certification.editCertificationName.path + '/' + id);
...
export default CertificationNameDetailPage;
