...
import { ButtonProps, Empty, Flex, Form, Input, Tooltip } from 'antd';
...
const breadcrumbItems = [
    ...
    { title: pathnames.admin.roleAndPermission.add.name }
];
...
const AddRolePage = () => {
    ...
    const pageTitle = () => {
        return (
            <div>
                <span style={{ marginRight: 5 }}> {pathnames.admin.roleAndPermission.add.name}</span>
    ...
    const onFinish = async (value: ICreateRole) => {
        ...
        try {
            ...
            const res = await roleAndPermissionService.createRole(body);
                    ...
                    return submenu.id === subMenuId
                        ? {
                              ...
                              sections: submenu.sections.map((section: any) =>
                                  section.id === sectionId ? { ...section, [fieldName]: value } : section
                              )
                          }
                        : submenu;
                }
    ...
    useEffect(() => {
        const fetchData = async () => {
            turnOnLoading();
            const res = await roleAndPermissionService.getAllIndex();
            const { data = [] } = res;
            const menuFormat = formatMenu(data);

            const updatedMenuFormat = menuFormat.map(menu => ({
                ...menu,
                subMenus: menu.subMenus?.map(subMenu => ({
                    ...subMenu,
                    sections: subMenu.sections.map((section: any) => ({
                        ...section,
                        isAccess: [74, 75, 76].includes(section.id) ? true : section.isAccess
                    }))
                }))
            }));

            setMenuOptions(updatedMenuFormat);
            turnOffLoading();
        };
